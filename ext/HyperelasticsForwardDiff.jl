module HyperelasticsForwardDiff
...
"""
function NonlinearContinua.SecondPiolaKirchoffStressTensor(ψ::Hyperelastics.AbstractHyperelasticModel, λ⃗::AbstractVector, p, ad_type::AutoForwardDiff; kwargs...)
...
- `adb`: Differentiation backend from `AbstractDifferentiation.jl`
"""
function NonlinearContinua.SecondPiolaKirchoffStressTensor(ψ::Hyperelastics.AbstractHyperelasticModel, F::AbstractMatrix, p, ad_type::AutoForwardDiff; kwargs...)
...
- `adb`: Differentiation backend from `AbstractDifferentiation.jl`
    """
function NonlinearContinua.CauchyStressTensor(ψ::Hyperelastics.AbstractHyperelasticModel, λ⃗::AbstractVector, p, ad_type::AutoForwardDiff; kwargs...)
    W(λ⃗) = StrainEnergyDensity(ψ, λ⃗, p)
    ∂W∂λ = gradient(W, λ⃗, kwargs...)
    σ = ∂W∂λ .* λ⃗ ./ J(λ⃗)
    return σ
...
- `adb`: Differentiation backend from `AbstractDifferentiation.jl`
"""
function NonlinearContinua.CauchyStressTensor(ψ::Hyperelastics.AbstractHyperelasticModel, F::AbstractMatrix, p, ad_type::AutoForwardDiff; kwargs...)
    B = F * F'
    a = eigvecs(B)'
