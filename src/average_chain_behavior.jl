...
"""
struct DataDrivenAverageChainBehavior{T,S} <: AbstractDataDrivenHyperelasticModel{PrincipalValueForm}
    data::T
    fchain::S
end
...
function NonlinearContinua.SecondPiolaKirchoffStressTensor(ψ::DataDrivenAverageChainBehavior, λ⃗::Vector, p; kwargs...)
    λch = sqrt(I₁(λ⃗) / 3)
    sᵢ = λ⃗ .* ψ.fchain(λch)
    return sᵢ
end

function NonlinearContinua.CauchyStressTensor(ψ::DataDrivenAverageChainBehavior, λ⃗::Vector, p; kwargs...)
    λch = sqrt(I₁(λ⃗) / 3)
