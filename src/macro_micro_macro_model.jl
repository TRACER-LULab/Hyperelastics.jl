...
"""
struct MacroMicroMacro{T} <: Hyperelastics.AbstractDataDrivenHyperelasticModel{PrincipalValueForm}
    Pchain::T
    r⃗::Vector{Vector{Float64}}
    w::Vector{Float64}

    function MacroMicroMacro(tests::Vector{<:Hyperelastics.AbstractHyperelasticTest}, Pchain, p₀; optimizer=LBFGS(), loss=L2DistLoss())
        ...
        λch = map(j->map(i->λchain(λ⃗[i], r⃗[j]), eachindex(λ⃗)), eachindex(r⃗))|>splat(hcat)
        # @tullio λch[i, j] := λchain(λ⃗[i], r⃗[j])

        # Pre-compute Coefficients for quadrature
        # @tullio wrᵢ²[k, i] := w[k] .* r⃗[k][i] .^ 2

        wrᵢ² = map(k->w[k].*r⃗[k].^2, eachindex(w))|>splat(hcat)

        function calc_stresses(test, Pchain)
            λ⃗ = test.data.λ
            ∂W∂λ = map(j->map(i->sum(k->wrᵢ²[k, j] .*- Pchain(λchain(λ⃗[i], r⃗[k]))), eachindex(λ⃗)), 1:size(wrᵢ², 2))
            @show size(∂W∂λ)
